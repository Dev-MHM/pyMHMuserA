    ...
    def load(self, log=True, func=import_module, cmd_help=HELP, include=[], exclude=[]):
        ...
        for plugin in sorted(files):
            ...
            try:
                ...
                doc = None
                self._logger.info(f"MHMuser - {self.key} - ERROR - {plugin}")
...
def load_other_plugins(addons=None, pmbot=None, manager=None, vcbot=None):
    ...
    if addons:
        ...
        if not os.path.exists("addons"):
            os.system(
                f"git clone -q -b {Repo().active_branch} https://github.com/Dev-MHM/pyMHMuserAddons.git addons"
            )
        else:
        ...
        if not os.path.exists("addons"):
            os.system(
                "git clone -q https://github.com/Dev-MHM/pyMHMuserAddons.git addons"
            )
