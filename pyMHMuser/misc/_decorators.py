...
def mhmuser_cmd(pattern=None, manager=False, **kwargs):
    ...
    def decor(dec):
        async def wrapp(ult):
            ...
            try:
                ...
                await mhmuser_bot.disconnect()
                await asyncio.sleep(fwerr.seconds + 10)
                await mhmuser_bot.connect()
                await asst.send_message(
                    int(udB.get_key("LOG_CHANNEL")),
                ...
                await asst.send_message(
                    ...
                    buttons=[
                        ...
                        Button.url(
                            "Repl",
                            "https://replit.com/@MHMuser/UltroidStringSession",
                        ),
                    ],
                )
                sys.exit()
                ...
                ftext = (
                    "**Ultroid Client Error:** `Forward this to` @MHMsupport\n\n"
                )
                ftext += "**py-MHMUser Version:** `" + str(pyver)
                ...
                if len(ftext) > 4096:
                    with BytesIO(ftext.encode()) as file:
                        ...
                        error_log = await asst.send_file(
                            ...
                            file,
                            caption="**Ultroid Client Error:** `Forward this to` @MHMsupport\n\n",
                        )
                else:
