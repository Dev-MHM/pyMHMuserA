...
async def randomchannel(
    tochat, channel, range1, range2, caption=None, client=mhmuser_bot
):
...
async def allcmds(event, telegraph):
    ...
    await eor(event, f"MHMuser Cmds : @MHMuser)", link_preview=False)
...
async def _insta_login():
    if "insta_creds" in mhmuser_bot._cache:
        return mhmuser_bot._cache["insta_creds"]
    ...
    if username and password:
        ...
        try:
            ...
            mhmuser_bot._cache.update({"insta_creds": cl})
...
async def get_insta_code(username, choice):
    from .. import asst, mhmuser_bot

    async with asst.conversation(mhmuser_bot.uid, timeout=60 * 2) as conv:
