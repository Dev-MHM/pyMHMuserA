...
async def metadata(file):
    ...
    try:
        ...
        data["duration"] = 69
        data["performer"] = udB.get_key("artist") or mhmuser_bot.me.first_name
    return data
...
async def get_chatbot_reply(message):
    ...
    req_link = chatbot_base.format(
        mhmuser_bot.me.first_name or "ultroid user",
        message,
    )
    try:
...
def telegraph_client():
    ...
    gd_name = mhmuser_bot.full_name
    short_name = gd_name[:30]
    profile_url = (
        f"https://t.me/{mhmuser_bot.me.username}" if mhmuser_bot.me.username else None
    )
    try:
